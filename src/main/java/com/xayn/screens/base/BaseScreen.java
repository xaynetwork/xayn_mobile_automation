...
public abstract class BaseScreen<T extends LoadableComponent<T>> extends LoadableComponent<T> {
    ...
    protected void click(List<MobileElement> mobileElementList, int index) {
        //   getWaitUtils().elementFromTheListToBeClickable(mobileElementList, index).click();
    }

    protected void clickWithVisibilityCheck(List<MobileElement> mobileElementList, int index) {
        // getWaitUtils().elementFromTheListToBeVisible(mobileElementList, index).click();
    }

    protected void click(MobileElement mobileElement, int times) {
        for (int i = 0; i < times; i++) {
            click(mobileElement);
        }
    }

    protected int getSize(List<MobileElement> mobileElementList) {
        try {
            //   getWaitUtils().elementFromTheListToBeVisible(mobileElementList, 0);
    ...
    public void setValue(List<MobileElement> mobileElementList, int index, String value) {
        //  getWaitUtils().waitElementFromTheListToExist(mobileElementList, index).setValue(value);
    }

    public void setValue(MobileElement mobileElement, String value) {
        //getWaitUtils().waitUntilElementVisible().setValue(value);
    }

    public boolean isTextContainsOnTheScreen(String text) {
        return getDriver().getPageSource().contains(text);
    }


    public boolean isTextContains(MobileElement mobileElement, String text) {
        return getText(mobileElement).contains(text);
    }

    protected boolean isElementChecked(MobileElement mobileElement) {
        String element = mobileElement.getAttribute("checked");
    ...
    protected void clickAndType(MobileElement element, String value) {
        click(element);
        cleanText(element);
        type(element, value);
    }
